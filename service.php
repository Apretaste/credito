...
class Service
{
	...
	public function _main (Request $request, Response $response)
	{
		...
		$items = Connection::query("
			...
			WHERE A.sender = '{$request->person->email}'
			AND A.transfered = '1' 
	...
	public function _procesar(Request $request, Response $response)
	{
		...
		Connection::query("
			...
			VALUES ('{$request->person->email}','{$person->email}','$price','$confirmationHash','$code')");
	...
	public function _aceptar(Request $request, Response $response)
	{
		...
		Connection::query("
			START TRANSACTION;
			UPDATE person SET credit=credit-{$transfer->amount} WHERE email='{$request->person->email}';
			UPDATE person SET credit=credit+{$transfer->amount} WHERE email='{$transfer->receiver}';
